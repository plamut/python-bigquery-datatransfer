...
class TestDataTransferServiceClient(object):
    ...
    def test_update_transfer_config(self):
        ...
        assert len(channel.requests) == 1
        expected_request = datatransfer_pb2.UpdateTransferConfigRequest(transfer_config=transfer_config, update_mask=update_mask)
        actual_request = channel.requests[0][1]
        assert expected_request == actual_request
    ...
    def test_schedule_transfer_runs(self):
        ...
        assert len(channel.requests) == 1
        expected_request = datatransfer_pb2.ScheduleTransferRunsRequest(parent=parent, start_time=start_time, end_time=end_time)
        actual_request = channel.requests[0][1]
        assert expected_request == actual_request
    ...
    def test_get_transfer_run(self):
        ...
        schedule = 'schedule-697920873'
        notification_pubsub_topic = 'notificationPubsubTopic1794281191'
        expected_response = {'name': name_2, 'destination_dataset_id': destination_dataset_id, 'data_source_id': data_source_id, 'user_id': user_id, 'schedule': schedule, 'notification_pubsub_topic': notification_pubsub_topic}
